...
function createBaseTRequestParams(): TRequestParams {
  return { DisableOutputSpeech: false, DisableShouldListen: false };
}

export const TRequestParams = {
  encode(message: TRequestParams, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Reader | Uint8Array, length?: number): TRequestParams {
    const reader = input instanceof Reader ? input : new Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseTRequestParams();
  ...
  fromJSON(object: any): TRequestParams {
    return {
  ...
  toJSON(message: TRequestParams): unknown {
    const obj: any = {};
