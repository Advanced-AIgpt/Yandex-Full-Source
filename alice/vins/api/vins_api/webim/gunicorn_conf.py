...
def on_starting(server):
    from vins_api.common.db import get_redis_connection
    from vins_api.webim import api
    from vins_api.webim import settings
    from vins_core.utils.metrics import sensors, RedisMetricsStorage
    ...
    for app_id in preload_app.split(';'):
        ...
        if app_id:
            ...
            resource_name = settings.CONNECTED_APPS.get(app_id, {}).get('resource', 'webim')
...
def post_request(worker, req, environ, resp):
    from vins_api.webim import api
    resource = api.app_resources.get('webim_v2', None)
    if resource is not None:
        resource.respond_to_user(req)
    resource = api.app_resources.get('ocrm_v2', None)
    if resource is not None:
        resource.respond_to_user(req)
