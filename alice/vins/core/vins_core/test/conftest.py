...
def _get_simple_app(test_entity_parser, num_classifiers=1):
    ...
    if num_classifiers == 1:
        intent_3_trainable_classifiers = ["test", "test_fallback"]
    else:
        intent_3_trainable_classifiers = ['test', 'test_2', 'test_fallback']
    intent_3 = Intent(
        name='custom_entity_test',
        trainable_classifiers=intent_3_trainable_classifiers,
        dm_form=Form(
            name='custom_entity_test',
            slots=[
    ...
    app_cfg.nlu = {
        ...
        'fallback_intent_classifiers': fallback_intent_classifiers,
        'utterance_tagger': {
            'model': 'crf',
            'name': 'tagger',
...
def test_entity_parser():
    resource = get_resource_full_path('resource://custom_enitities/custom_enitities/test_entities.tar.gz')
...
@pytest.fixture(name='app_config', scope='function')
...
def normalizing_samples_extractor(normalize_sample_processor):
    return SamplesExtractor(pipeline=[normalize_sample_processor], allow_wizard_request=True)
