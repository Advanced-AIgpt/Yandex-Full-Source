...
from crm_bot.app import CrmBotApp
from personal_assistant.testing_framework import parse_placeholders, init_mock
from crm_bot.testing_framework import get_vins_response, load_testcase
...
def hardcoded_responses():
    ...
    with mock.patch('personal_assistant.hardcoded_responses.load_hardcoded', return_value=resp):
        yield resp
...
def vins_app(fixlist, banlist, hardcoded_responses, gc_banlist):
    ...
    with mock.patch('vins_core.ext.s3.S3DownloadAPI.get_if_modified', get_s3_data):
        vins_application = CrmBotApp(
            vins_file='crm_bot/config/Vinsfile.json',
...
def vins_response_provider(vins_app):
    return functools.partial(get_vins_response, vins_app, str(gen_uuid_for_tests()))
