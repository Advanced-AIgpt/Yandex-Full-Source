...
from alice.analytics.operations.timespent.mappings import get_scenario_from_view_source, NOT_IN_TLT_TVT_MUSIC_SCENARIO, \
    NOT_JOINED_MUSIC_SCENARIO
...
from alice.analytics.operations.timespent.device_mappings import map_expboxes_tv_device
...
def make_job(job, nirvana, options):
    ...
    logs = (
        job
            ...
            .project(ne.all(exclude=["scenario", "device"]),
                     device=ne.custom(lambda app, device: device if
                     app != 'tv'
                     else map_expboxes_tv_device(device), 'app', 'device').with_type(typing.Optional[typing.String]),
                     scenario=ne.custom(lambda scenario, tlt_tvt: scenario if
                     (tlt_tvt == 0 or (tlt_tvt > 0 and scenario in TLT_TVT_SCENARIOS))
                     else NOT_IN_TLT_TVT_MUSIC_SCENARIO, 'scenario', 'tlt_tvt').with_type(
                         typing.Optional[typing.String]))
            .concat(tv)
    )
    ...
    (
        timespent
            ...
            .join(weekly_devices.project(ne.all(exclude=['devices']), weekly_devices='devices'), type='left',
