...
def parse_cmd_args():
    ...
    parser.add_argument('-cd', '--col_descr', dest='column_description', help='column description', required=True)
...
def wrapper_target_func(f, q, prefeatures_list, number, for_fitting, max_num_answers):
    prefeatures = collections.defaultdict(lambda: collections.defaultdict(lambda: collections.defaultdict(dict)))
    while True:
        try:
            work = q.get(timeout=0)
        except queue.Empty:
            break
        f(work, prefeatures, for_fitting, max_num_answers)
        q.task_done()
    prefeatures_copy = dict()
    for task_id, item0 in  prefeatures.items():
        prefeatures_copy[task_id] = dict()
        for cost, item1 in item0.items():
            prefeatures_copy[task_id][cost] = dict()
            for position, item2 in item1.items():
                prefeatures_copy[task_id][cost][position] = item2

    prefeatures_list[number] = prefeatures_copy
...
def process(context):
    ...
    write_result(context.column_description, cd)
...
class FeaturesExtractor:
    ...
    @staticmethod
    def _extract_row_prefeatures(row, prefeatures, for_fitting, max_num_answers):
        toloka_answers = sorted(row['raw_assesments'], key=lambda x: x['submitTs'])
        id_ = row['inputValues']['audio']
        if for_fitting:
            cost_range = range(1, min(max_num_answers, len(toloka_answers)) + 1)
        else:
            cost_range = [min(max_num_answers, len(toloka_answers))]
    ...
    def _extract_prefeatures(self):
        ...
        for row in self.rows:
            q.put_nowait(row)
        prefeatures_list = manager.list()
        pool = list()
        for i in range(20):
            prefeatures_list.append(
                None
            )
            pool.append(Process(target=wrapper_target_func,
                        args=(
                            self._extract_row_prefeatures, q,
                            prefeatures_list,
                            i,
                            self.for_fitting,
                            self.max_num_answers)
                                ))
            pool[-1].start()
        q.join()
        for p in pool:
            p.join()
        prefeatures = prefeatures_list[0]
        for part in prefeatures_list[1:]:
            prefeatures.update(part)
        logging.debug('prefeatures extracted for {} rows'.format(len(prefeatures)))
        return prefeatures
