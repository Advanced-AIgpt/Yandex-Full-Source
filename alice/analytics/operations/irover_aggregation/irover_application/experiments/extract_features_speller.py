...
class Processor:
    def __init__(self, assignments, context, fixed_words, fixed_texts):
...
#         self.prepare_data()
    ...
    def _extract_prefeatures(self):
        ...
        for index, row in enumerate(tqdm_notebook(self.rows)):
            ...
            for cost in cost_range:
                ...
                for text, speech, worker_id in [(x['text'], x['speech'], x['worker_id']) for x in
                                                toloka_answers[:cost]]:
#                     for i, answer in enumerate(row['raw_assesments']):
#                         if answer['worker_id'] == worker_id:
#                             self.rows[index]['raw_assesments'][i]['text'] = text
#                             break
                    if cost < self.MAX_NUM_ANSWERS:
                        fixed_text = self.fixed_texts.get(text, text)
                        if fixed_text != text:
                            speller_answer = {
                            ...
                            self.rows[index]['raw_assesments'].append(speller_answer)
                            toloka_answers.append(speller_answer)
...
#                 mv_hyp = self.majority_vote_text(hyps)
#                 ref = WordTransitionNetwork(object_id=id_, hypotheses=[TextHyp(id_, 'reference', ref_text)])
    ...
    def _get_word_features(self, word):
        ...
        result = [
            len(word), 
            self._count_eng_letters(word), 
            len(word) - self._count_eng_letters(word), 
            word == '?', 
            word in self.fixed_words
        ]
        return result
