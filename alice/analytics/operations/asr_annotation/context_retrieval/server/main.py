    ...
    class CustomHandler(http.server.BaseHTTPRequestHandler):
        ...
        def return_markup_page(self, name, id):
            ...
            with open(paths[id], encoding='utf-8') as fin:
                html = fin.read()
            html = html.replace('SIDESPEECH_VALUE', '{}|{}|sidespeech'.format(name, id))
            html = html.replace('REQUEST_VALUE', '{}|{}|request'.format(name, id))
            html = html.replace('SKIP_VALUE', '{}|{}|skip'.format(name, id))
            html = html.replace('DUMP_VALUE', name)
            self.wfile.write(html.encode('utf-8'))
