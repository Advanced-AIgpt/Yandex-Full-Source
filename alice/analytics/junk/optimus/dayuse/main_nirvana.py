...
CLI_OPTIONS = [
    cli.Option('job_date'),
    cli.Option('update_period', default=7),
    cli.Option('last_dayuse_table_path', default=None),
]

@cli.statinfra_job(options=CLI_OPTIONS)
def make_job(job, nirvana, options):
    job_date = options.job_date
    update_period = int(options.update_period)
    ...
    last_dayuse_table_path = options.last_dayuse_table_path
