...
@Component
public class ReadNextNewsCallbackProcessor extends BaseFlashBriefingCallbackProcessor {
    ...
    protected ReadNextNewsCallbackProcessor(
    ...
    public boolean canProcess(MegaMindRequest<DialogovoState> request) {
        return onCallbackDirective(NewsReadNextDirective.class)
                .test(request);
    }

    @Override
    public RunRequestProcessorType getType() {
        return RunRequestProcessorType.EXTERNAL_SKILL_NEWS_READ_NEXT;
    }

    @Override
    protected NewsPayload getNewsPayload(MegaMindRequest<DialogovoState> request) {
        NewsReadNextDirective payload = request.getInput().getDirective(NewsReadNextDirective.class);
    ...
    protected Optional<NewsContent> getNewsContent(List<NewsContent> topNewsByFeed, Optional<String> contentIdO,
                                                   int depth) {
        return newsContentsResolver.findNextOne(topNewsByFeed, contentIdO, depth);
    }
