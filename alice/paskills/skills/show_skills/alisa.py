...
class Alisa:
    ...
    def is_widget_gallery_request(self):
        return self.event['request']['type'] == "WidgetGallery"

    def is_teasers_request(self):
        return self.event['request']['type'] == "Teasers"

    def is_button_pressed_request(self):
        return self.event['request']['type'] == "ButtonPressed"

    def get_payload(self):
        return self.event['request']['payload']
    ...
    def show_episode(self, text, episode_id=None, title=None, tts=None, title_tts=None, pub_date=None, exp_date=None):
        ...
        if exp_date:
            show_item_meta['expiration_date'] = exp_date
    ...
    def widget(self, text, image_id):
            widget_gallery_meta = {}
            widget_gallery_meta['text'] = text
            widget_gallery_meta['image_id'] = image_id

            # inmplemented for poem of the day without 'text' field
            # button = {}
            # button['title'] = 'Слушать'
            # widget_gallery_meta['buttons'] = [button]

            tap_action = {}
            tap_action['payload'] = {"text": text}
            widget_gallery_meta['tap_action'] = tap_action

            self.answer['text'] = ""
            self.answer['widget_gallery_meta'] = widget_gallery_meta

    def teasers(self, text, image_id):
        teaser = {'text': text, 'image_id': image_id}
        teasers_meta = [teaser]

        tap_action = {'payload': {"text": text}}
        teaser['tap_action'] = tap_action

        self.answer['teasers_meta'] = teasers_meta
