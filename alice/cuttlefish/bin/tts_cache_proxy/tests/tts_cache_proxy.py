...
TTS_CACHE_PROXY_BIN_PATH = yatest.common.binary_path("alice/cuttlefish/bin/tts_cache_proxy/tts_cache_proxy")


class TtsCacheProxy(test_lib.ApphostServant):
    @property
    def eventlog_path(self):
        return yatest.common.test_output_path("tts_cache_proxy.evlog")

    @property
    def rtlog_path(self):
        return yatest.common.test_output_path("tts_cache_proxy.rtlog")

    def __init__(self, bin_path=TTS_CACHE_PROXY_BIN_PATH, env={}, args=[]):
    ...
    def _after_start(self):
        ...
        logging.info("gRPC apphost client for tts cache proxy is ready")

    def create_apphost_grpc_stream(self, path=ServiceHandles.TTS_CACHE, **kwargs):
    ...
    async def make_grpc_request(self, items, timeout=None, path=ServiceHandles.TTS_CACHE, **kwargs):
