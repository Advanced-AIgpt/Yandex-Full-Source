...
type MigrationDBClient struct {
	*db.DBClient
	FixedScenariosCounter int
}

func (db *MigrationDBClient) changeRecord(ctx context.Context, record userScenario) error {
	newDevices := make([]model.ScenarioDevice, 0, len(record.Scenario.Devices))
	needChange := false
	for _, device := range record.Scenario.Devices {
		deviceToAppend := device
		deviceToAppend.Capabilities = make([]model.ScenarioCapability, 0)
		for _, capability := range device.Capabilities {
			if capability.Type == "" {
				logger.Infof("Found bad capability in scenario: id: %s, name: %s, user: %d", record.Scenario.ID, record.Scenario.Name, record.User.ID)
				needChange = true
				continue
			}
			deviceToAppend.Capabilities = append(deviceToAppend.Capabilities, capability)
		}
		newDevices = append(newDevices, deviceToAppend)
	}
	if needChange {
		record.Scenario.Devices = newDevices
		if err := db.UpdateScenario(ctx, record.User.ID, record.Scenario); err != nil {
			return err
		}
		db.FixedScenariosCounter++
	}
	return nil
}
...
func do(ctx context.Context, client *MigrationDBClient) error {
	...
	logger.Infof("fixed scenarios: %d", client.FixedScenariosCounter)
	logger.Info("done")
	return nil
}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to eliminate empty capabilities from user Scenarios at `%s%s`", endpoint, prefix)
