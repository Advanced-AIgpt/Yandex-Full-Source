...
func do(ctx context.Context, client *MigrationDBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("properties", ydb.Optional(ydb.TypeJSON)),
	}
	...
	batchSize := 1000
	devicesChannel := client.StreamDevicesWithoutProperties(ctx)
	var batchNumber int
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column properties to table Devices at `%s%s`", endpoint, prefix)
