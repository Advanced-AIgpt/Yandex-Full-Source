...
func do(ctx context.Context, client *db.DBClient, tableName string) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("steps", ydb.Optional(ydb.TypeJSON)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column steps to table %s at `%s%s`", tableName, endpoint, prefix)
	...
	if err := do(ctx, dbcli, tableName); err != nil {
