...
func (db *MigrationDBClient) SetUpdatedToCreated(ctx context.Context) error {
	query := fmt.Sprintf(`
		--!syntax_v1
	...
	if err := db.Write(ctx, query, nil); err != nil {
		return xerrors.Errorf("failed to set updated to created on devices: %w", err)
	}
...
func do(ctx context.Context, client *MigrationDBClient) error {
	if err := client.SetUpdatedToCreated(ctx); err != nil {
		return err
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to set updated to created in table Devices at `%s%s`", endpoint, prefix)
