...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("launch_trigger_value", ydb.Optional(ydb.TypeJSON)),
	}

	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.AlterTable(ctx, path.Join(client.Prefix, "ScenarioLaunches"), tableOptions...); err != nil {
			return err
		}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column launch_trigger_value to table ScenarioLaunches at `%s%s`", endpoint, prefix)
