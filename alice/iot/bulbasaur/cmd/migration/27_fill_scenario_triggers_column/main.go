...
func (db *MigrationDBClient) GetScenariosChunk(ctx context.Context, lastScenario *Scenario) ([]Scenario, bool, error) {
	query := fmt.Sprintf(`
		...
		$scenarios = (
			SELECT huid, id, name, scheduled, trigger_type FROM Scenarios
			WHERE triggers IS NULL AND huid = $lastHuid AND id > $lastId
			ORDER BY huid, id LIMIT $limit

			UNION ALL

			SELECT huid, id, name, scheduled, trigger_type FROM Scenarios
			WHERE triggers IS NULL AND huid > $lastHuid
			ORDER BY huid, id LIMIT $limit
		);

		SELECT huid, id, name, scheduled, trigger_type FROM $scenarios ORDER BY huid, id LIMIT $limit
	`, db.Prefix)
	...
	for res.NextRow() {
		...
		res.NextItem()
		name := string(res.OString())

		res.NextItem()
		scheduled := res.OTimestamp()

		res.NextItem()
		triggerType := string(res.OString())
...
func (db *MigrationDBClient) UpdateScenarioTriggers(ctx context.Context, scenarios []Scenario) (int, error) {
	updateParams := make([]ydb.Value, 0, len(scenarios))
	for _, s := range scenarios {
		var triggers []model.ScenarioTrigger
		if s.triggerType == string(model.VoiceScenarioTriggerType) {
			triggers = []model.ScenarioTrigger{
				model.VoiceScenarioTrigger{Phrase: s.name},
			}
		} else if s.triggerType == string(model.TimerScenarioTriggerType) {
			triggers = []model.ScenarioTrigger{
				model.TimerScenarioTrigger{Time: timestamp.FromMicro(s.scheduled)},
			}
		} else {
			logger.Infof("Unknown trigger type (huid: %d, id: %s): %s", s.huid, s.id, s.triggerType)
			continue
		}
...
func do(ctx context.Context, client *MigrationDBClient) error {
	...
	for hasMoreData {
		...
		updatedCount, err := client.UpdateScenarioTriggers(ctx, chunk)
		if err != nil {
			return err
		}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to fill triggers column for Scenarios at `%s%s`", endpoint, prefix)
