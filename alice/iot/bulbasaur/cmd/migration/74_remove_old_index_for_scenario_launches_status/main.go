...
func do(ctx context.Context, client *db.DBClient) error {
	query := fmt.Sprintf(`
		...
		ALTER TABLE ScenarioLaunches
		DROP INDEX scenario_launches_huid_trigger_type_status;
	`, client.Prefix)

	session, err := client.YDBClient.TableClient.CreateSession(ctx)
	if err != nil {
		return err
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to drop index scenario_launches_huid_trigger_type_status from table ScenarioLaunches at `%s%s`", endpoint, prefix)
