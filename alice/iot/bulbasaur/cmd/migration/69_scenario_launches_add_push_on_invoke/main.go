...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("push_on_invoke", ydb.Optional(ydb.TypeBool)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column push_on_invoke to table ScenarioLaunches at `%s%s`", endpoint, prefix)
