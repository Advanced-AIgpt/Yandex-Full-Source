...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("current_step_index", ydb.Optional(ydb.TypeInt64)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column current_step_index to table ScenarioLaunches at `%s%s`", endpoint, prefix)
