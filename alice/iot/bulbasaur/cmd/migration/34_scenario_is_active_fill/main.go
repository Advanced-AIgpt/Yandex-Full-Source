...
func do(ctx context.Context, client *MigrationDBClient) error {
	batchSize := 1000
	scenariosChannel := client.StreamScenarios(ctx)
	var batchNumber int
	...
	for {
		...
		if len(scenarios) > 0 {
			...
			go func(batchNumber int) {
				defer wg.Done()
				if err := client.FillIsActiveOnScenarios(ctx, scenarios); err != nil {
	...
	wg.Wait()
	logger.Infof("Filling is_active column done, finished on batch %d", batchNumber)
	return nil
}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to fill nulls of is_active column by value true at table Scenarios at `%s%s`", endpoint, prefix)
