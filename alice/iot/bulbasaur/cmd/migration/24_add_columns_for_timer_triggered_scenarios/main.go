...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("scheduled", ydb.Optional(ydb.TypeTimestamp)),
		table.WithAddColumn("status", ydb.Optional(ydb.TypeString)),
		table.WithAddColumn("error", ydb.Optional(ydb.TypeString)),
	}
