...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("short_address", ydb.Optional(ydb.TypeString)),
	}

	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.AlterTable(ctx, path.Join(client.Prefix, "Households"), tableOptions...); err != nil {
			return err
		}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column short_address to table Households at `%s%s`", endpoint, prefix)
