...
func (db *MigrationDBClient) FillUpdatedOnDevices(ctx context.Context) error {
	query := fmt.Sprintf(`
		--!syntax_v1
		...
		UPDATE
			Devices
		SET
			updated = CAST(0 As Timestamp)
		WHERE
			updated is NULL;
	`, db.Prefix)

	if err := db.Write(ctx, query, nil); err != nil {
		return xerrors.Errorf("failed to fill updated on devices: %w", err)
	}
...
func do(ctx context.Context, client *MigrationDBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("updated", ydb.Optional(ydb.TypeTimestamp)),
	}

	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.AlterTable(ctx, path.Join(client.Prefix, "Devices"), tableOptions...); err != nil {
			return err
		}
	...
	err = client.FillUpdatedOnDevices(ctx)
	if err != nil {
		return err
	}
	logger.Infof("Filling updated column done\n")
	return nil
}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column updated to table Devices at `%s%s`", endpoint, prefix)
