...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("effective_time", ydb.Optional(ydb.TypeJSON)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column effective_time to table Scenarios at `%s%s`", endpoint, prefix)
