...
func do(ctx context.Context, client *db.DBClient, tableName string) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("household_id", ydb.Optional(ydb.TypeString)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column household_id to tables Devices, Groups and Rooms at `%s%s`", endpoint, prefix)
	...
	if err := do(ctx, dbcli, "Devices"); err != nil {
		logger.Fatal(err.Error())
	}
	if err := do(ctx, dbcli, "Groups"); err != nil {
		logger.Fatal(err.Error())
	}
	if err := do(ctx, dbcli, "Rooms"); err != nil {
		logger.Fatal(err.Error())
	}
}
