...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("archived_at", ydb.Optional(ydb.TypeTimestamp)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column archived_at to table Devices at `%s%s`", endpoint, prefix)
