...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("household_name", ydb.Optional(ydb.TypeString)),
	}

	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.AlterTable(ctx, path.Join(client.Prefix, "SharedHouseholds"), tableOptions...); err != nil {
			return err
		}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column household_name to table SharedHouseholds at `%s%s`", endpoint, prefix)
