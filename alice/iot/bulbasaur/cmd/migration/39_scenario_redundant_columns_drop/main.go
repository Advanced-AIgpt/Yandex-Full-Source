...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithDropColumn("trigger_type"),
		table.WithDropColumn("external_actions"),
		table.WithDropColumn("scheduled"),
		table.WithDropColumn("status"),
		table.WithDropColumn("error"),
	}
	...
	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		...
		logger.Infof("drop column done\n")
		return nil
	}))

	if err != nil {
		logger.Errorf("drop column went wrong: %v", err)
		return err
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to drop columns (trigger_type, scheduled, status, error, external_actions) from table Scenarios at `%s%s`", endpoint, prefix)
