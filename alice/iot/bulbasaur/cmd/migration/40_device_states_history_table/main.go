	...
	tableOptions := []table.CreateTableOption{
		...
		table.WithTimeToLiveSettings(
			table.TimeToLiveSettings{
				ColumnName:         "ts",
				ExpireAfterSeconds: uint32((90 * 24 * time.Hour).Seconds()),
			},
		),
		table.WithProfile(
			table.WithPartitioningPolicy(
				table.WithPartitioningPolicyUniformPartitions(32),
			),
		),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to create table DeviceStatesHistory at `%s%s`", endpoint, prefix)
