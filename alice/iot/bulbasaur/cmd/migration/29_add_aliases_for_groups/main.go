...
func do(ctx context.Context, client *db.DBClient) error {
	...
	err := table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.AlterTable(ctx, path.Join(client.Prefix, "Groups"), tableOptions...); err != nil {
			return err
		}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add aliases column to table Groups at `%s%s`", endpoint, prefix)
