...
func do(ctx context.Context, client *db.DBClient) error {
	query := fmt.Sprintf(`
		...
		ADD INDEX scenario_launches_huid_status
		GLOBAL ON (huid, status);
	`, client.Prefix)
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add index to table ScenarioLaunches at `%s%s`", endpoint, prefix)
