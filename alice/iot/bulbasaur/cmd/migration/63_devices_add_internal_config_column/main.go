...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("internal_config", ydb.Optional(ydb.TypeJSON)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column internal_config to table Devices at `%s%s`", endpoint, prefix)
