...
func do(ctx context.Context, client *db.DBClient) (err error) {
	...
	return table.Retry(ctx, client.SessionPool, table.OperationFunc(func(ctx context.Context, s *table.Session) error {
		if err := s.DropTable(ctx, tableName); err != nil {
			return err
		}
		logger.Infof("drop table done\n")
		return nil
	}))
}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to drop table StationOwners at `%s%s`", endpoint, prefix)
