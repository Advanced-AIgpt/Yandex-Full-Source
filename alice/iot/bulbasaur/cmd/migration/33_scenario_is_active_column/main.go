...
func do(ctx context.Context, client *db.DBClient) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("is_active", ydb.Optional(ydb.TypeBool)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column is_active to table Scenarios at `%s%s`", endpoint, prefix)
