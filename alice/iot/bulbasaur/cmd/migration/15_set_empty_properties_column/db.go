...
func (db *MigrationDBClient) StreamTuyaSockets(ctx context.Context) <-chan Device {
	...
	go func() {
		...
		res, err := s.StreamReadTable(ctx, devicesTablePath,
			...
			table.ReadColumn("original_type"),
		)
		if err != nil {
		...
		for res.NextStreamSet(ctx) {
			for res.NextRow() {
				...
				res.SeekItem("original_type")
				originalType := string(res.OString())
				...
				if skillID == "T" && originalType == string(model.SocketDeviceType) {
