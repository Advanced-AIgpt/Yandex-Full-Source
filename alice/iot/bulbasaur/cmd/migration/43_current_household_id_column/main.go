...
func do(ctx context.Context, client *db.DBClient, tableName string) error {
	tableOptions := []table.AlterTableOption{
		table.WithAddColumn("current_household_id", ydb.Optional(ydb.TypeString)),
	}
...
func main() {
	...
	msg := fmt.Sprintf("Do you really want to add column current_household_id to table Users at `%s%s`", endpoint, prefix)
	...
	if err := do(ctx, dbcli, "Users"); err != nil {
