...
class TtsClient:
    def __init__(self, params={}):
        ...
        self.tts_stream = TtsStream(
            self.on_tts_stream_result,
            self.on_tts_stream_error,
            params=self.params,
            host="localhost"
        )
    ...
    def on_tts_stream_result(self, result, *args, **kwargs):
        self.results.put_nowait(result)

    def on_tts_stream_error(self, err):
...
def count_in_hgram_interval(name, hgrams, hgram_interval):
    name_hgram = "{}_hgram".format(name)
    for hgram in hgrams:
        if hgram[0] != name_hgram:
            continue
        total = 0
        for i in hgram[1]:
            if hgram_interval[0] <= i[0] < hgram_interval[1]:
                total += i[1]
        return total
    return 0
