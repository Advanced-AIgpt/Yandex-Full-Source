...
def _check_text(response, entries):
    ...
    text = response.text
    assert text.startswith('Включаю')
...
class TestPalmMusicSearch(object):
    ...
    @pytest.mark.parametrize('command_object, response_snippet, first_track_title, answer_type', [
        ...
        pytest.param(
            ...
            'The Show Must Go On',
            'Queen, трек "The Show Must Go On"',
            'Track',
        ...
        pytest.param(
            ...
            'Ни там ни тут',
            'Агата Кристи, трек "Ни там ни тут"',
            'Track',
        ...
        pytest.param(
            'современную музыку',
            ['подборку', re.compile(r'Громкие новинки месяца|Чарт Яндекс Музыки')],  # XXX(a-square): bug?
            None,
            'Playlist',
            id='playlist_modern',
        ),
